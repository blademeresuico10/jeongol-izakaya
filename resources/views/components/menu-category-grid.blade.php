<div class="flex flex-col">
  ...
  <div class="w-full overflow-x-hidden">
    <div class="grid gap-2 mt-1
      {{ $key === 'main' ? 'grid-cols-2 main-menu-grid' : 'grid-cols-2 sm:grid-cols-3 md:grid-cols-3' }}">
      ...
      @foreach($items as $item)
        ...
        <div
          class="menu-card bg-white border border-gray-300 rounded-md shadow hover:shadow-md hover:bg-gray-100 cursor-pointer transition duration-200"
          onclick="selectMenuItem(this)" data-id="{{ $item->id }}" data-name="{{ $baseName }}" data-category="{{ $key }}"
          data-price="{{ $item->price }}">
          <div class="menu-image-container w-full h-24 overflow-hidden rounded-t-md">
