...
<body class="bg-gray-100 min-h-screen font-sans">
          ...
          <div class="flex items-center justify-between">
            <div class="flex items-center space-x-4">
              <div class="flex items-center space-x-3">
                ...
                <div>
                  <h3 class="customer-name text-lg font-bold text-gray-900">{{ $order->customer_name ?? 'Unknown' }}</h3>
                  <p class="text-sm text-gray-500">
                    {{ ucfirst($order->source ?? 'Customer') }}
                  </p>
                </div>
              </div>
              ...
              <div class="flex items-center space-x-2">
                <div class="bg-green-100 px-3 py-1 rounded-full">
                  <span class="text-green-800 font-semibold text-sm">Table {{ $order->table_number ?? 'N/A' }}</span>
                </div>
              </div>
              ...
              <div class="flex items-center space-x-2">
                ...
                <span class="text-gray-700 font-medium">{{ $order->pax ?? 0 }} pax</span>
              </div>
            </div>

            <button type="button" 
              data-record-id="{{ $order->record_id }}" 
              data-source="{{ $order->source }}"
              data-pax="{{ $order->pax }}"
              data-orders="{{ htmlspecialchars($order->orders, ENT_QUOTES) }}" 
              data-order-data="{{ htmlspecialchars($order->order_data, ENT_QUOTES) }}"
  ...
  <script>
    function openModal(button) {
      ...
      document.getElementById('record_id').value = button.getAttribute('data-record-id');
